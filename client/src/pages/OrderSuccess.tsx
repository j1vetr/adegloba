...
export default function OrderSuccess() {
    ...
    return (
      <UserShell hideBottomNav hideHeader>
        <div className="min-h-[80vh] flex flex-col items-center justify-center text-center">
          ...
          <p className="text-sm text-slate-500">{t.orderSuccess.checkingOrder}</p>
        </div>
      </UserShell>
    );
  }
  ...
  if (error || (!verifiedOrder && orderDetails?.orderId)) {
    return (
      <UserShell title={t.orderSuccess.orderNotFound} showBack backTo="/panel">
        <div className="user-card-elevated p-6 text-center">
          ...
          <h2 className="text-lg font-bold text-slate-900 mb-1">{t.orderSuccess.orderNotFound}</h2>
          <p className="text-sm text-slate-500 mb-4">{t.orderSuccess.orderNotVerified}</p>
  ...
  const steps = [t.orderSuccess.stepCart, t.orderSuccess.stepPayment, t.orderSuccess.stepConfirm];

  return (
    <UserShell title={t.orderSuccess.paymentSuccessful} hideBottomNav>
      <div className="space-y-4">
        ...
        <div className="user-card-elevated p-6 text-center">
          ...
          <h1 className="text-xl font-bold text-slate-900 mb-1.5">{t.orderSuccess.paymentReceived}</h1>
          <p className="text-sm text-slate-500 mb-5">{t.orderSuccess.accountActivated}</p>
            ...
            <div className="rounded-xl bg-slate-50 border border-slate-100 p-3 mb-5 text-left text-sm space-y-1.5">
              {orderDetails?.orderId && (
                <div className="flex justify-between items-center gap-2">
                  <span className="text-slate-500">{t.orderSuccess.orderId}</span>
                  <button
                    ...
                    aria-label={t.orderSuccess.orderId}
                  >
              ...
              {(verifiedOrder?.totalUsd || orderDetails?.amount) && (
                <div className="flex justify-between gap-2">
                  <span className="text-slate-500">{t.orderSuccess.amount}</span>
                  <span className="text-emerald-600 font-bold">${verifiedOrder?.totalUsd || orderDetails?.amount}</span>
                </div>
              )}
              {verifiedOrder?.status && (
                <div className="flex justify-between gap-2">
                  <span className="text-slate-500">{t.orderSuccess.status}</span>
                  <span className="chip chip-success">{t.orderSuccess.paid}</span>
                </div>
              )}
              {orderDetails?.paymentId && (
                <div className="flex justify-between gap-2">
                  <span className="text-slate-500">{t.orderSuccess.paymentId}</span>
          ...
          <div className="space-y-2">
            ...
            >
              {t.orderSuccess.viewActivePackage} <ArrowRight className="h-4 w-4" />
            </button>
          </div>
        </div>

        <p className="text-center text-xs text-slate-400">
          {t.orderSuccess.supportContact}
        </p>
      </div>
    </UserShell>
  );
