...
export const emailCampaigns = pgTable("email_campaigns", {
  ...
  targetType: varchar("target_type").notNull(), // 'all', 'ship', 'users'
  targetIds: text("target_ids").array(), // Array of ship IDs or user IDs
  status: varchar("status").notNull().default('draft'), // 'draft', 'scheduled', 'sent', 'cancelled'
...
export const emailTemplates = pgTable("email_templates", {
  ...
  subject: varchar("subject").notNull(),
  content: text("content").notNull(),
  type: varchar("type").notNull().default('marketing'), // 'marketing', 'transactional', 'welcome'
  isActive: boolean("is_active").default(true),
  createdBy: varchar("created_by").references(() => admin_users.id),
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow(),
...
export const campaignLogs = pgTable("campaign_logs", {
  ...
  status: varchar("status").notNull(), // 'sent', 'delivered', 'opened', 'clicked', 'bounced', 'failed'
  errorMessage: text("error_message"),
...
export const emailCampaignsRelations = relations(emailCampaigns, ({ one, many }) => ({
  createdByAdmin: one(admin_users, { fields: [emailCampaigns.createdBy], references: [admin_users.id] }),
  logs: many(campaignLogs),
}));

export const emailTemplatesRelations = relations(emailTemplates, ({ one }) => ({
  createdByAdmin: one(admin_users, { fields: [emailTemplates.createdBy], references: [admin_users.id] }),
}));

export const campaignLogsRelations = relations(campaignLogs, ({ one }) => ({
  campaign: one(emailCampaigns, { fields: [campaignLogs.campaignId], references: [emailCampaigns.id] }),
