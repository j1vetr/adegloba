...
export interface IStorage {
  ...
  getDashboardRevenue(days: number): Promise<Array<{ date: string; revenue: number; orders: number }>>;
  getPopularPackages(): Promise<Array<{ name: string; count: number; revenue: number }>>;
  getRecentActivities(limit: number): Promise<Array<{ id: string; type: string; message: string; timestamp: string; status: string }>>;
...
export class DatabaseStorage implements IStorage {
  ...
  async getDashboardRevenue(days: number): Promise<Array<{ date: string; revenue: number; orders: number }>> {
    try {
  ...
  async getPopularPackages(): Promise<Array<{ name: string; count: number; revenue: number }>> {
    try {
      const popularPackages = await db
        .select({
          name: plans.name,
          count: sql<number>`COUNT(${orderItems.id})`,
          revenue: sql<number>`COALESCE(SUM(CAST(${orderItems.totalUsd} AS DECIMAL) * ${orderItems.qty}), 0)`
        })
        .from(orderItems)
      ...
      return popularPackages.map(row => ({
        name: row.name,
  ...
  async getRecentActivities(limit: number): Promise<Array<{ id: string; type: string; message: string; timestamp: string; status: string }>> {
    try {
      const activities: Array<{ id: string; type: string; message: string; timestamp: string; status: string }> = [];
      ...
      recentOrders.forEach(order => {
        if (order.createdAt) {
          activities.push({
            id: order.id,
            type: 'order',
            message: `${order.username} placed an order ($${order.totalUsd})`,
            timestamp: order.createdAt.toISOString(),
            status: order.status === 'paid' || order.status === 'completed' ? 'success' : 
